...
# Naive implementation of connecting to ybmex websocket for streaming realtime data.
...
class ybmexWebsocket:
    ...
    #     key_list = self.
    #     self.ws.send(json.dumps({"op": "authKeyExpires", "args": args}))
        ...
        try:
            if 'subscribe' in message:
                ...
                if action == 'partial':
                    ...
                    if table not in ['order', 'orderBookL2'] and len(self.data[table]) > ybmexWebsocket.MAX_TABLE_LEN:
                        self.data[table] = self.data[table][int(ybmexWebsocket.MAX_TABLE_LEN / 2):]
                elif action == 'update':
